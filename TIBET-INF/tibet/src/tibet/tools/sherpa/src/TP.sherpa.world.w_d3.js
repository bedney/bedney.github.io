...
TP.sherpa.world.Inst.defineAttribute('$zoomer');
TP.sherpa.world.Inst.defineAttribute('$zoomerTarget');
    ...
    var zoomListener = d3.behavior.zoom().scaleExtent([0.1, 3]).on('zoom',
            function() {
                ...
                transformStr = 'translate(' +
                                    ...
                                    d3.event.translate[1] + 'px' +
                                    ') ' +
                                    'scale(' + d3.event.scale + ')';

                TP.elementSetTransform(natElem, transformStr);
            });

    var zoomerTarget = d3.select(natElem.ownerDocument.body);
    zoomListener(zoomerTarget);

    zoomListener.scale(0.5);

    this.set('$zoomer', zoomListener);
    this.set('$zoomerTarget', zoomerTarget);
...
function(screenRowNum, screenColNum) {
    ...
    if (TP.isValid(hud = TP.byOID('SherpaHUD')) &&
        ...
        //translateX += 30 + (gapWidth * screenColNum);
        translateX += 30 + gapWidth;
        translateY += 50;
    }

    this.get('$zoomer').translate(TP.ac(translateX, translateY));
    this.get('$zoomer').scale(this.getScaleFactor());

    this.get('$zoomer').event(this.get('$zoomerTarget').transition().duration(500));
...
TP.sherpa.world.Inst.defineMethod('getScaleFactor',
function() {
    ...
    windowWidth = TP.documentGetViewableWidth(this.getNativeDocument());

    screenWidth = this.get('screenWidth');
    gapWidth = this.get('gapWidth');

    scale = windowWidth / (screenWidth + (gapWidth * 2) + hudWidthOffset);
...
function(aRect) {
    ...
    centerPoint = TP.pc(centerPoint.getX(), centerPoint.getY());
