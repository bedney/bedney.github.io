...
function(uniqueID, dataRecord) {
    ...
    //  We put the raw data on the output element as an 'expando' property
    outElem.rawData = dataRecord.at('rawData');
...
function(uniqueID) {
    ...
    TP.elementSetAttribute(elem, 'draggable', true);

    consoleDNDElem = this.get('$dndElem');

    hudNatElem = TP.byOID('SherpaHUD').getNativeNode();
    ...
    elem.addEventListener(
                ...
                function(evt) {
                    ...
                    TP.$$DNDObj = this.rawData;

                    rawDataID = TP.id(this.rawData);

                    evt.dataTransfer.setData(TP.ietf.Mime.PLAIN, rawDataID);
                    TP.nodeSetTextContent(consoleDNDElem, rawDataID);

                    evt.dataTransfer.setDragImage(consoleDNDElem, 0, 0);

                    TP.elementGetStyleObj(hudNatElem).pointerEvents = 'auto';

                    console.log('hey, I started: ' + evt.target);
                },
                false);
    ...
    elem.addEventListener(
                ...
                function(evt) {
                    ...
                    TP.elementGetStyleObj(hudNatElem).pointerEvents = 'none';

                    TP.$$DNDObj = null;

                    console.log('hey, I ended: ' + evt.target);
                },
                false);
...
TP.sherpa.console.Inst.defineMethod('createDNDElement',
function() {
    ...
    consoleDNDElem = TP.documentCreateElement(doc, 'div');
    TP.elementSetClass(consoleDNDElem, 'output-drag-element');

    TP.nodeAppendChild(consoleDNDElem, doc.createTextNode(''), false);
    TP.nodeAppendChild(doc.body, consoleDNDElem, false);

    this.set('$dndElem', consoleDNDElem);
